...
const char * const CEREBRALCOIN_CONF_FILENAME = "cerebralcoin.conf";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "cerebralcoind -foo=bar
...
#else
    const char* pszModule = "cerebralcoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Cerebralcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Cerebralcoin
    // Mac: ~/Library/Application Support/Cerebralcoin
    // Unix: ~/.cerebralcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cerebralcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Cerebralcoin";
#else
    // Unix
    return pathRet / ".cerebralcoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CEREBRALCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Cerebralcoin Core copyright is not removed by accident
    if (copyright_devs.find("Cerebralcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Cerebralcoin Core developers";
    }
