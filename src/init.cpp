 ...
 */
static const char* CEREBRALCOIN_PID_FILENAME = "cerebralcoind.pid";

static fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", CEREBRALCOIN_PID_FILENAME)));
}
 ...
 * between cerebralcoind, and cerebralcoin-qt and non-server tools.
*/
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("cerebralcoin-shutoff");
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify configuration file. Relative paths will be prefixed by datadir location. (default: %s)", CEREBRALCOIN_CONF_FILENAME), false, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", CEREBRALCOIN_PID_FILENAME), false, OptionsCategory::OPTIONS);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/cerebralcoin/cerebralcoin>";
    const std::string URL_WEBSITE = "<https://cerebralcoincore.org>";
...
/** Sanity checks
 *  Ensure that Cerebralcoin is running in a usable environment with all
...
{
    // Make sure only a single Cerebralcoin process is using the data directory.
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", CEREBRALCOIN_CONF_FILENAME));
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if cerebralcoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if cerebralcoin is started while in a temporary directory.\n",
