#!/usr/bin/env bash
# Copyright (c) 2016 The Cerebralcoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   cerebralcoincore.org and cerebralcoin.org and compares them.
...
WORKINGDIR="/tmp/cerebralcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://cerebralcoincore.org"
HOST2="https://cerebralcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="cerebralcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'cerebralcoin-',
...
if [ $? -ne 0 ]; then
   echo "cerebralcoin.org failed to provide signature file, but cerebralcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "cerebralcoin.org and cerebralcoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Cerebralcoin Core binary release signing key installed?"
   fi
